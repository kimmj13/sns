...
public class PostServiceTest {
    ...
    void 포스트수정이_성공한경우() {
        ...
        PostEntity fixture = PostEntityFixture.get(userName, postId, 1);
        UserEntity user = fixture.getUser();

        when(userEntityRepository.findByUserName(userName)).thenReturn(Optional.of(user));
        when(postEntityRepository.findById(postId)).thenReturn(Optional.of(fixture));
        when(postEntityRepository.saveAndFlush(any())).thenReturn(fixture);
    ...
    void 포스트수정시_권한이_없는_경우() {
        ...
        when(postEntityRepository.findById(fixture.getId())).thenReturn(Optional.of(fixture));
